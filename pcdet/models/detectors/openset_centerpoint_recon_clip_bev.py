...
class Openset_CenterPoint_Recon_clip_BEV(Detector3DTemplate):
    ...
    def get_training_loss(self, batch_dict):
        ...
        clip_features = batch_dict["clip_input_sp_tensor"]
        bev_clip_features = clip_features.dense()
        N, C, D, H, W = bev_clip_features.shape
        bev_clip_features = bev_clip_features.view(N, C * D, H, W)
        bev_clip_features_flatten = bev_clip_features.contiguous().permute(1,0,2,3).contiguous().flatten(1).contiguous().permute(1,0)
        
        encoded_clip_features = self.encoder(bev_clip_features_flatten)
        recon_features = self.decoder(encoded_clip_features)
 
        
        clip_features_norm = (bev_clip_features_flatten/(bev_clip_features_flatten.norm(dim=-1, keepdim=True)+1e-5))
        ...
        if CHECK_VIZ:
            ...
            #############################reconstruction######################################
            # recon_bev_image_clip_features = recon_features.view()
            ...
            recon_features = recon_features.permute(1,0)
            recon_image_clip_features_norm = (recon_features/(recon_features.norm(dim=0, keepdim=True)+1e-5)).half()
