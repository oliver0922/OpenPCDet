...
############# reconstruction 없이 768 dim으로 학습 ##############
CHECK_VIZ = True

class Openset_CenterPointV2(Detector3DTemplate):
    ...
    def forward(self, batch_dict):
       ...
       ########## decoder 활용해서 visualization ######################
        ...
        if CHECK_VIZ:
            ...
            flatten_predicted_bev_clip_feature = predicted_bev_clip_feature[0].contiguous().flatten(1)
            
            decoded_flatten_predicted_bev_clip_feature_norm = (flatten_predicted_bev_clip_feature/(flatten_predicted_bev_clip_feature.norm(dim=0, keepdim=True)+1e-5)).half()
            ...
            plt.savefig("/home/OpenPCDet/visualization/prediction/with_bg_prediction_from_centerpt_l2_768.png",dpi=300, bbox_inches='tight')
            plt.axis('off')
            plt.clf() 
            plt.close('all')        
    ...
    def get_clip_loss(self, batch_dict):
        ...
        clip_bev_features = batch_dict["clip_input_sp_tensor"].dense()
        N, C, D, H, W = clip_bev_features.shape
        clip_bev_features_reshaped = clip_bev_features.view(N, C * D, H, W)

        
        l2loss = self.l2_loss(point_clip_feature, clip_bev_features_reshaped)
        return l2loss
        # cosloss = self.cos_loss(point_clip_feature, clip_bev_features_reshaped)
        # return cosloss
        # l1_loss = self.l1_loss(point_clip_feature, clip_bev_features_reshaped)
        # return l1_loss 
