...
class SkipConnectionNet(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(512, 768, kernel_size=3, padding=1)
        self.bn3 = nn.BatchNorm2d(768)
        
        self.skip_conv1 = nn.Conv2d(64, 256, kernel_size=1)
        self.skip_bn1 = nn.BatchNorm2d(256)
        self.skip_conv2 = nn.Conv2d(256, 512, kernel_size=1)
        self.skip_bn2 = nn.BatchNorm2d(512)
        self.skip_conv3 = nn.Conv2d(512, 768, kernel_size=1)
        self.skip_bn3 = nn.BatchNorm2d(768)
        
    def forward(self, x):
        # First layer with skip connection
        skip1 = self.skip_bn1(self.skip_conv1(x))
        x = F.relu(self.bn1(self.conv1(x)), inplace=False)
        x = x + skip1
        
        # Second layer with skip connection
        skip2 = self.skip_bn2(self.skip_conv2(x))
        x = F.relu(self.bn2(self.conv2(x)), inplace=False)
        x = x + skip2
        
        # Third layer with skip connection
        skip3 = self.skip_bn3(self.skip_conv3(x))
    ...
    def __init__(self, input_channels, sep_head_dict, init_bias=-2.19, use_bias=False, norm_func=None, class_agnoistic=False, clip_train=False):
        ...
        for cur_name in self.sep_head_dict:
            ...
            if cur_name == 'clip' and clip_train:
                clip_conv = SkipConnectionNet()
                # fc_list.append
                #     nn.Sequential(
                #     nn.Conv2d(64, 128, kernel_size=3, stride=1, padding=1, bias=True),
                #     nn.BatchNorm2d(128) if norm_func is None else norm_func(128),
                #     nn.ReLU(),
                #     nn.Conv2d(128, 256, kernel_size=3, stride=1, padding=1, bias=True),
                #     nn.BatchNorm2d(256) if norm_func is None else norm_func(256),
                #     nn.ReLU(),                    
                #     nn.Conv2d(256, 768, kernel_size=3, stride=1, padding=1, bias=True),                   
                #     ))
...
class Openset_CenterHeadV2(nn.Module):
