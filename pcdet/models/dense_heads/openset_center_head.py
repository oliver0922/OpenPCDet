...
# from ..model_utils import openset_centernet_utils
...
# class Openset_SeparateHead(nn.Module):
...
# class Openset_CenterHead(nn.Module):
...
#         radius = openset_centernet_utils.gaussian_radius(dx, dy, min_overlap=gaussian_overlap)
...
#             openset_centernet_utils.draw_gaussian_to_heatmap(heatmap[cur_class_id], center[k], radius[k].item())
...
#                 batch_box_preds = openset_centernet_utils.decode_bbox_from_pred_dicts(
#                     pred_dict=pred_dict,
...
#     def generate_predicted_boxes(self, batch_size, pred_dicts, openset_voxel_indices):
...
#             final_pred_dicts = openset_centernet_utils.decode_bbox_from_heatmap(
...
#                 openset_voxel_indices = openset_voxel_indices if idx==2 else None
#             )
...
#     def forward(self, data_dict, text_feats=None, pred_feats=None):
...
#             ###############################################################################
#             openset_scores, openset_indices = self.return_clip_indicies(text_feats, pred_feats)
#             voxel_indices = data_dict["encoded_spconv_tensor"].indices
#             openset_voxel_indices = voxel_indices[openset_indices]
#             # class_id = 4 * torch.ones(openset_voxel_indices.shape[0],1).cuda() # 4 voxel
#             # openset_voxel_indices = torch.cat((openset_voxel_indices, class_id), dim=-1)
#             ###############################################################################

            
#             pred_dicts = self.generate_predicted_boxes(
#                 data_dict['batch_size'], pred_dicts, openset_voxel_indices[:,2:]
#             )
...
#     def return_clip_indicies(self, text_feats, pred_feats):
        
#         unknown_similarity_matrix = (100.0 * pred_feats.half() @ text_feats.T).softmax(dim=-1)
#         unknown_max_scores, unknwon_class_indices = torch.max(unknown_similarity_matrix, dim=1)
        
#         # known_similarity_matrix = (100.0 * pred_feats.half() @ predefined_label_feats.T).softmax(dim=-1)
#         # known_max_scores, known_indices = torch.max(known_similarity_matrix, dim=1)
        
        
        
#         openset_indices = torch.where((unknwon_class_indices == 4)) #4 -> bus
#         openset_scores = unknown_max_scores[openset_indices]
#         ######################### -> to do: score threshold 주어서 너무 낮은 것은 제외하도록 masking 하는 것도 고려
        
#         return openset_scores, openset_indices
...
class SeparateHead(nn.Module):
    def __init__(self, input_channels, sep_head_dict, init_bias=-2.19, use_bias=False, norm_func=None, class_agnoistic=False, clip_train=False):
        super().__init__()
        if class_agnoistic is False:
            self.sep_head_dict = sep_head_dict
            del(self.sep_head_dict['clip'])
        else:
            self.sep_head_dict = sep_head_dict
            if clip_train == False:
                del(self.sep_head_dict['clip'])
...
class Openset_CenterHead(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
        ...
        self.voxel_size = voxel_size
        self.feature_map_stride = self.model_cfg.TARGET_ASSIGNER_CONFIG.get('FEATURE_MAP_STRIDE', None)
        self.clip_train = self.model_cfg.CLIP_TRAIN
            ...
            if idx != 6: #for class specific head
                self.heads_list.append(
                    SeparateHead(
                        ...
                        use_bias=self.model_cfg.get('USE_BIAS_BEFORE_NORM', False),
                        norm_func=norm_func,
                        class_agnoistic=False,
                        clip_train = self.clip_train
                    )
                )
            else: #for class agnostic head
                self.heads_list.append(
                    SeparateHead(
                        input_channels=self.model_cfg.SHARED_CONV_CHANNEL,
                        sep_head_dict=cur_head_dict,
