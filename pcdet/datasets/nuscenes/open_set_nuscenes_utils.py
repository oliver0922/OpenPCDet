...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10, with_cam=False):
    ...
    #################edit ###################    
    lidar_term = 'lidar' #edit 'multisweep' if use multi sweep
    camera_names = ['CAM_FRONT_LEFT', 'CAM_FRONT_RIGHT', 'CAM_FRONT', 'CAM_BACK_LEFT', 'CAM_BACK_RIGHT', 'CAM_BACK']
    dataset_type = 'nuscenes'
    map = {1: 255, 5: 255, 7: 255, 8: 255, 10: 255, 11: 255, 13: 255, 19: 255, 20: 255, 0: 255,
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        open_seg_lidar_path = os.path.join(data_path, 'nuScene_lidarseg_split', mode, scene_name, lidar_term, f'{sample["timestamp"]}.bin')

        
        # lidar_path = self.lidar_list[index]

        # if dataset_type == 'nuscenes' and not (lidar_term =='multisweep'):
        #     point = np.fromfile(open_seg_lidar_path, dtype=np.float32).reshape(-1, 4)[:, :3]

        # Color images
        color_image_path_stem = open_seg_lidar_path.split('.bin')[0].replace(f'/{lidar_term}', '/color')
        color_image_paths = [color_image_path_stem + f'_{camera_name}.jpg' for camera_name in camera_names]
        

        # CLIP features 
        clip_feature_path = open_seg_lidar_path.split('.bin')[0].replace(f'/{lidar_term}', '/openseg') + '.npz'
        ...
        # Camera intrinsics
        K_path_stem = os.path.dirname(open_seg_lidar_path).replace(f'/{lidar_term}', '/K')
        intrinsic_paths = [K_path_stem + f'/{camera_name}.npy' for camera_name in camera_names]
        # intrinsics= [np.load(K_path_stem + f'/{camera_name}.npy') for camera_name in camera_names]

        # LiDAR pose
        lidar_pose_stem = open_seg_lidar_path.split('.bin')[0].replace(f'/{lidar_term}', '/lidar_pose')+'.txt'
        # lidar_pose = np.loadtxt(lidar_pose_stem + '.txt')

        # labels
        if mode != 'test':
            label_path = open_seg_lidar_path.split('.bin')[0].replace(f'/{lidar_term}', '/label') +'.label'
            # label = np.fromfile(label_path + '.label', dtype=np.uint8).reshape(-1, 1)
            # label = np.vectorize(map.__getitem__)(label)
        else : 
            label_path = None
            # label == None
        ...
        # Camera poses
        pose_stem = open_seg_lidar_path.split('.bin')[0].replace(f'/{lidar_term}', '/pose')
        pose_paths = [pose_stem + f'_{camera_name}.txt' for camera_name in camera_names]
        ...
        data_dict= {
                ...
                "intrinsics_path" : intrinsic_paths, 
                "clip_feature_path": clip_feature_path,
                # "mask": entire_mask,
                "pose_path" : pose_paths,
                "lidar_pose_path" : lidar_pose_stem, 
