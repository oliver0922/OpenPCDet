...
class Openset_NuScenesDataset(DatasetTemplate):
  ...
  12: 7, 22: 8, 23: 9, 24: 10, 25: 11, 26: 12, 27: 13, 28: 14, 30: 15}
    ...
    def open_seg_data(self,info):
        
        
        # sample = self.nusc.get('sample',info['token'])
        # scene = self.nusc.get('scene', sample['scene_token'])
        # scene_name = scene['name']
        open_seg_lidar_path = info['coords_path']

        
        # lidar_path = self.lidar_list[index]

        # if self.dataset_type == 'nuscenes' and not (self.lidar_term =='multisweep'):
        # point = np.fromfile(open_seg_lidar_path, dtype=np.float32).reshape(-1, 4)[:, :3]

        # Color images
        # color_image_path_stem = open_seg_lidar_path.split('.bin')[0].replace(f'/{self.lidar_term}', '/color')
        # color_image_paths = [color_image_path_stem + f'_{camera_name}.jpg' for camera_name in self.camera_names]
        

        # CLIP features 
        # clip_feature = torch.tensor(np.load(info['clip_feature_path'])['feat'], dtype=torch.float16)
        clip_feature = np.load(info['clip_feature_path'])['feat']


        # entire_mask features 
        # entire_mask = torch.tensor(np.load(info['clip_feature_path'])['mask_full'])
        entire_mask = np.load(info['clip_feature_path'])['mask_full']
        ...
        # Camera intrinsics
        # K_path_stem = os.path.dirname(open_seg_lidar_path).replace(f'/{self.lidar_term}', '/K')
        # intrinsics= [np.load(intrinsic_path) for intrinsic_path in info['intrinsics_path']]

        # LiDAR pose
        # lidar_pose_stem = open_seg_lidar_path.split('.bin')[0].replace(f'/{self.lidar_term}', '/lidar_pose')
        # lidar_pose = np.loadtxt(info['lidar_pose_path'])

        # labels
        # if self.mode != 'test':
        #     # label_path = open_seg_lidar_path.split('.bin')[0].replace(f'/{self.lidar_term}', '/label')
        #     label = np.fromfile(info['label_path'], dtype=np.uint8).reshape(-1, 1)
        ...
        # Camera poses
        # pose_stem = open_seg_lidar_path.split('.bin')[0].replace(f'/{self.lidar_term}', '/pose')
        # pose_paths = [pose_stem + f'_{camera_name}.txt' for camera_name in self.camera_names]


        # poses = [np.loadtxt(pose_path) for pose_path in info['pose_path']] 

        data_dict= {
                # "coords":torch.from_numpy(point), 
                # "colors_path": info['colors_path'],
                # "coords_path": open_seg_lidar_path,
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        import json
        from nuscenes.nuscenes import NuScenes
        from . import open_set_nuscenes_utils
        nusc = NuScenes(version=self.dataset_cfg.VERSION, dataroot=str(self.root_path), verbose=True)
        nusc_annos = open_set_nuscenes_utils.transform_det_annos_to_nusc_annos(det_annos, nusc)
        nusc_annos['meta'] = {
            'use_camera': False,
            'use_lidar': True,
            'use_radar': False,
        ...
        result_str, result_dict = open_set_nuscenes_utils.format_nuscene_results(metrics, self.class_names, version=eval_version)
...
def create_nuscenes_info(version, data_path, save_path, max_sweeps=10, with_cam=False):
    ...
    from pcdet.datasets.nuscenes import open_set_nuscenes_utils as nuscenes_utils
    ...
    if version == 'v1.0-test':
        ...
        with open(save_path / f'nuscenes_infos_{max_sweeps}sweeps_train_with_clip.pkl', 'wb') as f:
            pickle.dump(train_nusc_infos, f)
        with open(save_path / f'nuscenes_infos_{max_sweeps}sweeps_val_with_clip.pkl', 'wb') as f:
...
if __name__ == '__main__':
    ...
    if args.func == 'create_nuscenes_infos':
        ...
        nuscenes_dataset = Openset_NuScenesDataset(
            dataset_cfg=dataset_cfg, class_names=None,
            root_path=ROOT_DIR / 'data' / 'nuscenes',
            logger=common_utils.create_logger(), training=True
        )
        nuscenes_dataset.create_groundtruth_database(max_sweeps=10)
